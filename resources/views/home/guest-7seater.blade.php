...
@section('title', 'Car Rental | 7 Seaters')
...
@section('content') 
    ...
    <section class="all-cars-section">
        ...
        <p class="category-title pb-4">7 Seaters</p>
        ...
        <!-- CAR PRODUCT -->  
            @foreach($addcar as $item)
                @if ($item->is_active == true && $item->vehicle == '7 Seaters')
            ...
            @if ($addcar->where('is_active', true)->where('vehicle', '7 Seaters')->count() == 0)
