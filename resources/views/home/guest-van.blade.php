...
@section('content') 
    ...
    <section class="all-cars-section">
        ...
        <p class="category-title pb-4">Van</p>
        ...
        <!-- CAR PRODUCT -->  
            @foreach($addcar as $item)
                @if ($item->is_active == true && $item->vehicle == 'Van')
            ...
            @if ($addcar->where('is_active', true)->where('vehicle', 'Van')->count() == 0)
